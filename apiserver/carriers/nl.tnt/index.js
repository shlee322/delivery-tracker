...
function getTrack(trackId) {
  return new Promise((resolve, reject) => {
    axios.get('https://www.tnt.com/api/v3/shipment', {
      params: {
        con: trackId,
        searchType: 'CON',
      ...
      let shippingInformation = {
        from: {
          address: [info.originAddress.city, info.originAddress.country].filter(e => e).join(', '),
          time: info.originDate
        },
        to: {
          address: [info.destinationAddress.city, info.destinationAddress.country].filter(e => e).join(', '),
      ...
      info.events.forEach(event => {
        const eventInfo = {
          time: event.date,
          status: { id: parseStatusId(event.legacyCode), text:event.statusDescription },
          location: { name: [event.location.city, event.location.country].filter(e => e).join(', ') }
        }

...
function getTrack(trackId) {
  ...
  return new Promise((resolve, reject) => {
    axios.post('https://www.fedex.com/trackingCal/track', qs.stringify({
      data: JSON.stringify({
      ...
      info.scanEventList.forEach(event => {
        ...
        if(!shippingInformation.from.time && eventInfo.status.id === 'at_pickup') shippingInformation.from.time = eventInfo.time;

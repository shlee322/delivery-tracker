...
function getTime(location, date, time) {
  ...
  timeResult[1] = parseInt(timeResult[1])
  if(timeResult[1] === 12) timeResult[1] = 0
  if(timeResult[3] === 'P.M.') timeResult[1] = timeResult[1] + 12
...
function getTrack(trackId) {
  ...
  return new Promise((resolve, reject) => {
    axios.post('https://www.ups.com/track/api/Track/GetStatus?loc=en_US', {
      Locale: 'en_US',
      TrackingNumber: [trackId]
    }).then(res => {
      
      if(res.data.statusCode !== '200' || res.data.trackDetails[0].errorCode) {
        return reject({
          code: 404,
          message: res.data.trackDetails ? res.data.trackDetails[0].errorText : res.data.statusText,
        })
      }
      ...
      info.shipmentProgressActivities.forEach(event => {
        ...
        if(!shippingInformation.from.time && eventInfo.status.id === 'at_pickup') {

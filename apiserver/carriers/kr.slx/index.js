...
function getTrack(trackId) {
  return new Promise((resolve, reject) => {
    axios.get('http://www.slx.co.kr/delivery/delivery_number.php', {
      params: {
        param1: trackId
      }
    }).then(res => {
      ...
      if(tables.length === 0) {
        const errorDiv = document.querySelector('div[style="text-align:center;color:red"]')
      ...
      progresses.forEach(element => {
        ...
        shippingInformation.progresses.push({
          time: `${tds[0].textContent.replace(/\./g, '-').replace(' ', 'T')}:00+09:00`,
          location: {
            name: tds[1].textContent,
          },
          description: `배송자: ${tds[2].textContent} (${tds[3].textContent})`,
          status: { id: parseStatusId(tds[4].textContent), text: tds[4].textContent }
        })
      })

...
function getTrack(trackId) {
  return new Promise((resolve, reject) => {
    axios.get('https://kdexp.com/newDeliverySearch.kd', {
      params: {
        barcode: trackId
      }
    }).then(res => {
      ...
      res.data.items.forEach(progress => {
        shippingInformation.progresses.push({
          time: progress.reg_date !== '알수없음' ? progress.reg_date.replace(' ', 'T') + '+09:00' : undefined,
          status: { id: STATUS_ID_MAP[progress.stat] || 'in_transit', text: progress.stat },

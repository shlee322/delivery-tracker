...
function getTrack(trackId) {
  ...
  return new Promise((resolve, reject) => {
    axios.post('https://service.epost.go.kr/trace.RetrieveDomRigiTraceList.comm', qs.stringify({
      sid1: trackId,
    })).then(res => {
      ...
      if ( $informationTable.find('tr').length === 3 ) {
        reject({
          code: 404,
          message: trimString($informationTable.find('tr:nth-child(2)').eq(0).text()),
        })
      }
      ...
      $progressTable.find('tr').each((_, element) => {
        ...
        shippingInformation.progresses.push({
          time: `${td.eq(0).html().replace(/\./g, '-')}T${td.eq(1).html()}:00+09:00`,
          location: {

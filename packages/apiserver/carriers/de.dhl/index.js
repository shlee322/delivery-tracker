...
function getStatus(description) {
  for(let key of Object.keys(STATUS_ID_MAP)) {
    if(description.toLowerCase().indexOf(key) !== -1) return STATUS_ID_MAP[key];
  }
...
function getTrack(trackId) {
  ...
  return new Promise((resolve, reject) => {
    axios.get('https://www.logistics.dhl/shipmentTracking', {
      params: {
        AWB: trackId,
        countryCode: 'g0',
      ...
      info.checkpoints.forEach(checkpoint => {
        shippingInformation.progresses.unshift({
          time: getTime(trimString(checkpoint.location), checkpoint.date, checkpoint.time),

...
function getTrack(trackId) {
  return new Promise((resolve, reject) => {
    axios.post('http://www.honamlogis.co.kr/tracking_number.php', `SLIP_BARCD=${trackId}`).then(res => {
      ...
      let shippingInformation = {
        from: {
          name: data.S_CUST_NM.substr(0,1)+"*"+data.S_CUST_NM.substr(2,1)
        },
        to: {
          name: data.R_CUST_NM.substr(0,1)+"*"+data.R_CUST_NM.substr(2,1),
          address: data.R_ZIP_ADDR.substr(0,data.R_ZIP_ADDR.length-6)+"******"
        },
      ...
      data.TRACKING_DTL.forEach(info => {
        shippingInformation.progresses.push({
          time: info.SCAN_DM.substr(0,4) + '-' + info.SCAN_DM.substr(4,2) + '-' + info.SCAN_DM.substr(6,2) + 'T' + info.SCAN_DM.substr(8,2) + ':' + info.SCAN_DM.substr(10,2) + ':00+09:00',
          status: { id: STATUS_ID_MAP[info.SCANGB_NM] || 'in_transit', text: info.SCANGB_NM },

...
function getTrack(trackId) {
  ...
  return new Promise((resolve, reject) => {
    axios.get('http://www.ilyanglogis.com/functionality/popup_result.asp', {
      params: {
        hawb_no: trackId
      },
      ...
      progresses.forEach(element => {
        ...
        shippingInformation.progresses.push({
          time: `${trimString(tds[0].textContent)}T${trimString(tds[1].textContent)}:00+09:00`,
          location: { name: trimString(tds[3].textContent) },
          description: trimString(tds[4].textContent) ? `연락처: ${trimString(tds[4].textContent)}` : undefined,
          status: { id: STATUS_ID_MAP[trimString(tds[2].textContent)] || 'in_transit', text: trimString(tds[2].textContent) }
        })
      })

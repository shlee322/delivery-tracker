...
function getTrack(trackId) {
  ...
  return new Promise((resolve, reject) => {
    axios.post('https://storm-uat.ipc.be/storm/pages/fragments/public/itemtracking', qs.stringify({
      itemSearch: trackId,
      language: '1'
    })).then(res => {
      ...
      const error = table ? table.querySelector('tr:first-child td[colspan="3"]') : true
      if ( error ) {
        return reject({
          code: 404,
      ...
      table.querySelectorAll('tr').forEach(element => {
        ...
        shippingInformation.progresses.push({
          ...
          status: { id: parseStatusId(tds[1].textContent), text: tds[1].textContent }
        })
      })

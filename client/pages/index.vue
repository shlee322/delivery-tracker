  ...
  <div id="tracker">
    ...
    <header class="column tracking-header">
      ...
      <div class="tracking-illustration has-text-centered">
        <img v-if="error === null" src="@/assets/nocanvas.png" alt="No Canvas">
    ...
    <div v-if="error === null" class="tracking-detail">
      <section class="tracking-progresses">
        <transition-group name="fold">
          <div v-for="(progressesByDate, date) in progresses" :key="date" class="tracking-progress-entry">
        ...
        <transition-group v-show="!isProgressTooShort" name="fold">
          <a class="fold-button has-text-centered" :key="`test1`" @click="fold = !fold">
            <div v-if="fold">더보기&nbsp;<i class="fas fa-angle-down"></i></div>
            <div v-else>간략히&nbsp;보기&nbsp;<i class="fas fa-angle-up"></i></div>
          </a>
        </transition-group>
      ...
      <section class="tracking-info">
        ...
        <ul>
          ...
          <li>
            <h5>택배사</h5>
            <p>{{ carrier.name }} ({{ formatNumber(carrier.tel, 'International') }})</p>
          </li>
          <li>
            <h5>송장번호</h5>
            <p>{{ track }}</p>
    ...
    <div style="margin-top:20px; color: #585858; font-size: 0.8125rem;">
      <a class="github-button" href="https://github.com/shlee322/delivery-tracker" data-icon="octicon-star" data-show-count="true" aria-label="Star shlee322/delivery-tracker on GitHub">Star</a>
      Copyright (C) <a href="https://tracker.delivery">https://tracker.delivery</a>
...
export default {
  ...
  watch: {
    ...
    fold() {
      this.progresses = this.fold && !this.isProgressTooShort ?
        this.getProgressesByDate(this.info.progresses.slice(this.info.progresses.length-3))
    ...
    fetchData() {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          this.progresses = this.getProgressesByDate(this.isProgressTooShort ? res: res.slice(res.length-3))
          this.isLoading = false
        })
        .catch(err => {
          this.error = {
            statusCode: err.response.status,
            message: err.response.data.message ? err.response.data.message : '알 수 없는 에러입니다!',
          }
          this.isLoading = false
        })
    },
...
.fold-enter-active, .fold-leave-active {
  transition: display .9s, opacity .6.5s ease-in-out;
}
